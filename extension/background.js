...
/*
 * The toolbar action has no popup; clicking it is a health check.
 * Badge: green tick = daemon answered, amber "!" = it did not.
 */
if (chrome.action && chrome.action.onClicked) {
  chrome.action.onClicked.addListener(async () => {
    const reply = await call('/resolve?url=' + encodeURIComponent('codelink://health'), { method: 'GET' });
    const up = reply.ok || reply.kind === 'api' || reply.kind === 'http';
    try {
      await chrome.action.setBadgeText({ text: up ? 'ok' : '!' });
      await chrome.action.setBadgeBackgroundColor({ color: up ? '#57a143' : '#b26b00' });
      await chrome.action.setTitle({
        title: up ? 'codelink — daemon is up' : 'codelink — daemon is not running',
      });
