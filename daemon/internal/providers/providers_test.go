...
// ownerConfigJSON captures the namespace alongside the repository, and declares
// the repo-level PAGE forms — a tree listing and a landing page — that name a
// repository without naming any file.
const ownerConfigJSON = `{
  "version": 1,
  "extensionId": "abcdefghijklmnopabcdefghijklmnop",
  "providers": [
    {
...
func TestParseOwnerGroup(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "owner and repo are both captured",
			url:       "https://a.example.com/acme/synapses/blob/main/lib/x.go",
			wantOwner: "acme", wantRepo: "synapses",
		},
		{
			// The entry that matches captures a repo but no owner, and one
			// missing group must not drag the other down with it.
			name:      "a repo without an owner leaves owner empty",
			url:       "https://a.example.com/anon/synapses/lib/x.go",
			wantOwner: "", wantRepo: "synapses",
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
...
// TestParsedOwnerIsOmittedWhenAbsent pins the compatibility half of the echo:
// a provider that captures no owner must serialise exactly as it did before the
// field existed.
func TestParsedOwnerIsOmittedWhenAbsent(t *testing.T) {
	cfg, err := loadJSON(t, ownerConfigJSON)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	parsed, ok := cfg.Parse("https://a.example.com/anon/synapses/lib/x.go")
	if !ok {
		t.Fatal("Parse did not match")
	}
	...
	if strings.Contains(string(raw), `"owner"`) {
		t.Errorf("payload carries an owner key with nothing to put in it: %s", raw)
	}

	parsed, ok = cfg.Parse("https://a.example.com/acme/synapses/blob/main/lib/x.go")
	if !ok {
		t.Fatal("Parse did not match")
	}
	raw, err = json.Marshal(parsed)
