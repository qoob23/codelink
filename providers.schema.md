...
| `owner` | The namespace segment (org / user). Echo-only: it never affects matching, but it is returned in `parsed`, and the extension keys its per-repo settings on `host/owner/repo` (lowercased). |
